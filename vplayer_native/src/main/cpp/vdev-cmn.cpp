...
void vdev_avsync_and_complete(void *ctxt) {
    ...
    if (!(c->status & VDEV_PAUSE)) {
        ...
        } else if ((c->vpts == -1 || c->vpts == AV_NOPTS_VALUE) &&
                   ++c->completed_counter == COMPLETE_COUNTER) {
            c->status |= VDEV_COMPLETED;
...
//        av_log(NULL,AV_LOG_ERROR,"weechan video_time: %lld  audio_time: %lld",c->vpts , c->apts);
...
//    }
//    av_log(NULL, AV_LOG_ERROR, "weechan avdiff: %3d, sleep_time: %lld\n", avdiff,(c->vpts -c->apts ) * 500);
//    if (c->ticksleep > 0) av_usleep( 33.3*1000 );
    }
}
